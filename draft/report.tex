...
\subsection{漸化式の第2項目について}
各漸化式の第2項目には確率密度関数の勾配が存在しており, 遷移が確定的である場合にどう扱えば良いのかわかりませんでした...


% comment
\if0
考えているのは\eqref{continuous}のような確定系なので,~$\textrm{Pr}(s^{\prime}|s, u)$が何に置き換わるのかを考えています.
\fi

\begin{align*}
	\nabla_{\theta}V^{\theta}(s) &= \nabla_{\theta}[r(s, \mu(s|\theta))+\gamma V^{\theta}(s^{\prime}(\theta))] \\
	&= \nabla_{\theta}r(s, \mu(s|\theta)) + \gamma\nabla_{\theta}V^{\theta}(s^{\prime}(\theta)) \\
	~\\
	\nabla_{\theta}V^{\theta}(s^{\prime}(\theta)) &=  \lim_{h\to0} \frac{V^{\theta+h}(s^{\prime}(\theta+h)) - V^{\theta}(s^{\prime}(\theta))}{h} \\
	&= \lim_{h\to0}\left\{\frac{V^{\theta+h}(s^{\prime}(\theta+h)) - V^{\theta}(s^{\prime}(\theta+h))}{h} + \frac{V^{\theta}(s^{\prime}(\theta+h)) - V^{\theta}(s^{\prime}(\theta))}{h}\right\} \\
	&= \lim_{h\to0}\frac{V^{\theta+h}(s^{\prime}(\theta+h)) - V^{\theta}(s^{\prime}(\theta+h))}{h} + \lim_{h\to0}\frac{V^{\theta}(s^{\prime}(\theta+h)) - V^{\theta}(s^{\prime}(\theta))}{h} \\
	&= \nabla_{\theta}V^{\theta}(s^{\prime})|_{s^{\prime}=s^{\prime}(\theta)} + \nabla_{s^{\prime}}V^{\theta}(s^{\prime})\nabla_{\theta}s^{\prime}(\theta)
\end{align*}
